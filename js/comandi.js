/* =============================================================================
   HIROSHI · comandi.js — i comandi

   Qui ci sono le mani: tendine, cursori, pulsanti, e il ciclo lento che liscia
   i parametri e aggiorna le targhe. Il DISEGNO sta in `tavola.js` ed è un file
   a parte — non per lunghezza, ma perché la regola «il disegno è puro display,
   non ascolta nulla» smette di essere un commento e diventa una cosa che si
   vede dalla lista dei file: la tavola legge il modello e non tocca un
   ascoltatore, i comandi toccano il modello e non disegnano un pixel.

   I COMANDI SONO ELEMENTI HTML NATIVI, e funzionano identici col puntatore,
   col dito, col tasto Tab e con un lettore di schermo. Nel canvas di Rada
   questa scelta è costata fatica ed è quella giusta: un cursore disegnato è un
   cursore che nessuno può usare senza vederlo.

   LE ETICHETTE DICONO L'AZIONE, NON LO STATO — «Pausa», non «In ascolto». Lo
   stato lo racconta la riga in alto, il punto che pulsa e la tavola.

   I dizionari dei nomi stanno qui e non nella tavola: sono testi d'interfaccia,
   e la tavola li legge da qui — è l'unica cosa che le serve dai comandi.
============================================================================= */
...
/* ----------------------------------------------------------- il registratore
   ...
   con dentro le mani, l'esportazione rende il pezzo che l'apparecchio farebbe
   da solo. La differenza sta scritta sotto i pulsanti, dove serve. */
const btnPresa = el("presa"), btnEsporta = el("esporta");
let orologioPresa2 = null;

btnPresa.addEventListener("click", async () => {
  if (stoRegistrando()) {
    const buf = fermaPresa();
    clearInterval(orologioPresa2);
    btnPresa.textContent = "Registra la sessione";
    btnPresa.classList.remove("attivo");
    if (salvaComeWav(buf)) {
      el("cronometro").textContent = numero(buf.duration, 1) + " s salvati";
    } else {
      el("cronometro").textContent = "non è arrivato niente";
    }
    return;
  }
  try {
    await avviaPresa();
    btnPresa.textContent = "Ferma e salva";
    btnPresa.classList.add("attivo");
    orologioPresa2 = setInterval(() => {
    ...
    el("cronometro").textContent = "non riesco ad aprire la presa";
  }
});

btnEsporta.addEventListener("click", async () => {
  const secondi = Number(el("durata").value);
  ...
  try {
    ...
    const buf = await esporta(secondi);
    salvaComeWav(buf);
